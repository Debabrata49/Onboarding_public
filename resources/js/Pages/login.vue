  ...
  <script>
    ...
    export default{
        setup(){
            ...
            let error = ref('')
            const login = async() =>{
                await axios.post('/api/loginOnboardingDashboard',form).then(res=>{
                  if (!res.data.error) {
                    // Store the token and expiration time in localStorage
                    const accessToken = res.data.message.original.access_token;
                    const expiresIn = res.data.message.original.expires_in; // expiration in seconds
                    const expirationTime = Math.floor(Date.now() / 1000) + expiresIn; // current time + expires_in
